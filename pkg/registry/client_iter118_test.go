...
func TestSignedWrappersIncludeCorrectChallenge(t *testing.T) {
	cases := []struct {
		...
		{"set_task_exec", func(c *Client) (map[string]interface{}, error) { return c.SetTaskExec(42, true) }, "set_task_exec:42"},
...
// --- Polo score: Update / Set / Get -------------------------------------

func TestUpdateAndSetPoloScoreDeltas(t *testing.T) {
	c, _ := echoOnlyClient(t)
	resp, _ := c.UpdatePoloScore(4, -5)
	echo := assertEcho(t, resp)
	if got, _ := echo["delta"].(float64); got != -5 {
		t.Fatalf("delta: %v", got)
	}

	resp, _ = c.SetPoloScore(4, 100)
	echo = assertEcho(t, resp)
	if got, _ := echo["polo_score"].(float64); got != 100 {
		t.Fatalf("polo_score: %v", got)
	}
}

func TestGetPoloScoreHappyAndMissing(t *testing.T) {
	var returnScore bool
	srv := newFakeJSONServer(t, func(_ map[string]interface{}) map[string]interface{} {
		if returnScore {
			return map[string]interface{}{"polo_score": float64(37)}
		}
	...
	returnScore = true
	n, err := c.GetPoloScore(9)
	if err != nil || n != 37 {
		t.Fatalf("expected (37, nil), got (%d, %v)", n, err)
	}

	returnScore = false
	n, err = c.GetPoloScore(9)
	if err == nil || !strings.Contains(err.Error(), "polo_score not found") {
		t.Fatalf("expected missing error, got (%d, %v)", n, err)
	}
}

func TestGetPoloScorePropagatesSendError(t *testing.T) {
	srv := newFakeJSONServer(t, func(_ map[string]interface{}) map[string]interface{} {
...
func TestAdminWrappersIncludeAdminToken(t *testing.T) {
	...
	cases := []struct {
		...
		{"set_task_exec_admin", func() (map[string]interface{}, error) { return c.SetTaskExecAdmin(1, true, "T") }, "set_task_exec"},
