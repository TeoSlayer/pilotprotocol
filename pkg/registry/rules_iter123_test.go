...
func TestValidateRulesLinksRequired(t *testing.T) {
	cases := []int{0, -5}
	for _, l := range cases {
		r := &NetworkRules{Links: l, Cycle: "1h", PruneBy: "score", FillHow: "random"}
		err := ValidateRules(r)
...
func TestValidateRulesCycleRequired(t *testing.T) {
	r := &NetworkRules{Links: 5, Cycle: "", PruneBy: "score", FillHow: "random"}
	err := ValidateRules(r)
...
func TestValidateRulesCycleInvalidDuration(t *testing.T) {
	r := &NetworkRules{Links: 5, Cycle: "not-a-duration", PruneBy: "score", FillHow: "random"}
	err := ValidateRules(r)
...
func TestValidateRulesCycleTooShort(t *testing.T) {
	r := &NetworkRules{Links: 5, Cycle: "30s", PruneBy: "score", FillHow: "random"}
	err := ValidateRules(r)
...
func TestValidateRulesPruneFillNegativeOrOverflow(t *testing.T) {
	base := NetworkRules{Links: 5, Cycle: "1h", PruneBy: "score", FillHow: "random"}
	// Prune < 0
	r := base
	r.Prune = -1
	if err := ValidateRules(&r); err == nil || !strings.Contains(err.Error(), "prune must be >= 0") {
		t.Fatalf("prune<0: %v", err)
	}
	// Fill < 0
	r = base
	r.Fill = -1
	if err := ValidateRules(&r); err == nil || !strings.Contains(err.Error(), "fill must be >= 0") {
		t.Fatalf("fill<0: %v", err)
	}
	// Prune > Links
	r = base
	r.Prune = 10
	if err := ValidateRules(&r); err == nil || !strings.Contains(err.Error(), "cannot exceed links") {
		t.Fatalf("prune>links: %v", err)
	}
	// Fill > Links
	r = base
	r.Fill = 10
	if err := ValidateRules(&r); err == nil || !strings.Contains(err.Error(), "fill (10) cannot exceed links") {
...
func TestValidateRulesPruneByAllValidValues(t *testing.T) {
	for _, pb := range []string{"score", "age", "activity"} {
		r := &NetworkRules{Links: 5, Cycle: "1h", PruneBy: pb, FillHow: "random"}
...
func TestValidateRulesPruneByRequiredAndUnknown(t *testing.T) {
	// Empty
	r := &NetworkRules{Links: 5, Cycle: "1h", PruneBy: "", FillHow: "random"}
	if err := ValidateRules(r); err == nil || !strings.Contains(err.Error(), "prune_by is required") {
		t.Fatalf("empty prune_by: %v", err)
	}
	// Unknown
	r = &NetworkRules{Links: 5, Cycle: "1h", PruneBy: "lottery", FillHow: "random"}
	if err := ValidateRules(r); err == nil || !strings.Contains(err.Error(), "unknown prune_by strategy") {
		t.Fatalf("unknown prune_by: %v", err)
	}
}

func TestValidateRulesFillHowRequiredAndUnknown(t *testing.T) {
	r := &NetworkRules{Links: 5, Cycle: "1h", PruneBy: "score", FillHow: ""}
	if err := ValidateRules(r); err == nil || !strings.Contains(err.Error(), "fill_how is required") {
		t.Fatalf("empty fill_how: %v", err)
	}
	r = &NetworkRules{Links: 5, Cycle: "1h", PruneBy: "score", FillHow: "roundrobin"}
	if err := ValidateRules(r); err == nil || !strings.Contains(err.Error(), "unknown fill_how strategy") {
		t.Fatalf("unknown fill_how: %v", err)
	}
}

func TestValidateRulesGraceInvalid(t *testing.T) {
	r := &NetworkRules{Links: 5, Cycle: "1h", PruneBy: "score", FillHow: "random", Grace: "not-a-duration"}
	if err := ValidateRules(r); err == nil || !strings.Contains(err.Error(), "invalid grace duration") {
...
func TestValidateRulesGraceEmptyOrValid(t *testing.T) {
	r := &NetworkRules{Links: 5, Cycle: "1h", PruneBy: "score", FillHow: "random", Grace: ""}
	if err := ValidateRules(r); err != nil {
...
func TestValidateRulesHappyPath(t *testing.T) {
	r := &NetworkRules{Links: 10, Cycle: "1h", Prune: 2, PruneBy: "score", Fill: 2, FillHow: "random", Grace: "5m"}
	if err := ValidateRules(r); err != nil {
...
func TestParseRulesInvalidRules(t *testing.T) {
	_, err := ParseRules(`{"links":0,"cycle":"1h","prune_by":"score","fill_how":"random"}`)
	if err == nil || !strings.Contains(err.Error(), "links must be >= 1") {
		t.Fatalf("%v", err)
	}
}
...
func TestParseRulesHappyPath(t *testing.T) {
	r, err := ParseRules(`{"links":5,"cycle":"1h","prune":1,"prune_by":"age","fill":1,"fill_how":"random"}`)
	if err != nil {
		t.Fatalf("%v", err)
	}
...
func TestRulesToPolicyShapeAndContentWithoutGrace(t *testing.T) {
	r := &NetworkRules{Links: 7, Cycle: "2h", Prune: 3, PruneBy: "age", Fill: 2, FillHow: "random"}
	raw, err := RulesToPolicy(r)
	if err != nil {
		t.Fatalf("%v", err)
	}
	...
	// rule[0] = score-activity on datagram
	r0 := rules[0].(map[string]interface{})
	if r0["name"].(string) != "score-activity" || r0["on"].(string) != "datagram" {
		t.Fatalf("rule 0: %+v", r0)
	}
	// rule[1] = cycle-prune-fill; prune action first, fill action second
...
func TestRulesToPolicyIncludesGraceWhenSet(t *testing.T) {
	r := &NetworkRules{Links: 5, Cycle: "1h", Prune: 1, PruneBy: "score", Fill: 1, FillHow: "random", Grace: "15m"}
	raw, err := RulesToPolicy(r)
	if err != nil {
		t.Fatalf("%v", err)
	}
...
func TestAllowedPortsToPolicyEmptyReturnsNilNil(t *testing.T) {
	raw, err := AllowedPortsToPolicy(nil)
	if err != nil || raw != nil {
		t.Fatalf("nil ports: raw=%v err=%v", raw, err)
	}
	raw, err = AllowedPortsToPolicy([]uint16{})
	if err != nil || raw != nil {
...
func TestAllowedPortsToPolicyMatchExpressionAndRules(t *testing.T) {
	raw, err := AllowedPortsToPolicy([]uint16{80, 443, 7001})
	if err != nil {
		t.Fatalf("%v", err)
	}
...
func TestAllowedPortsToPolicySinglePort(t *testing.T) {
	raw, err := AllowedPortsToPolicy([]uint16{7})
	if err != nil {
		t.Fatalf("%v", err)
	}
