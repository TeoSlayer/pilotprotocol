...
func TestRelayOnlyResolveSubstitutesBeacon(t *testing.T) {
	...
	s.trustPairs[trustPairKey(100, 200)] = true
	s.mu.Unlock()

	resp, err := s.handleResolve(map[string]interface{}{
		"node_id":      float64(200),
...
func TestRelayOnlyResolveNormalNodeUnchanged(t *testing.T) {
	...
	s.trustPairs[trustPairKey(300, 400)] = true
	s.mu.Unlock()
...
func TestRelayOnlyLookupSubstitutesBeacon(t *testing.T) {
	...
	resp, err := s.handleLookup(map[string]interface{}{
		"node_id": float64(500),
	})
	if err != nil {
