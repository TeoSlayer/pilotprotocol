...
func TestNewWALErrorOnBadPath(t *testing.T) {
	...
	if _, err := NewWAL(filepath.Join(file, "child.wal")); err == nil {
...
func TestAppendNilSafe(t *testing.T) {
	var w *WAL
	if err := w.Append(DeltaEntry{SeqNo: 1}); err != nil {
...
func TestReplayNilSafe(t *testing.T) {
	var w *WAL
	n, err := w.Replay(func(DeltaEntry) error { return nil })
	if err != nil || n != 0 {
...
func TestAppendWritesLengthPrefixedEntry(t *testing.T) {
	...
	entry := DeltaEntry{SeqNo: 7, Type: DeltaRegister, NodeID: 42, Data: json.RawMessage(`{"hello":"world"}`)}
	...
	if got.SeqNo != 7 || got.Type != DeltaRegister || got.NodeID != 42 {
...
func TestAppendAccumulatesSize(t *testing.T) {
	...
	s1 := w.Size()
	if err := w.Append(DeltaEntry{SeqNo: 2}); err != nil {
		t.Fatal(err)
	}
	s2 := w.Size()
	if s2 <= s1 {
...
func TestReplayReadsAllAppendedEntries(t *testing.T) {
	...
	for i := uint64(1); i <= 3; i++ {
		if err := w.Append(DeltaEntry{SeqNo: i, Type: DeltaHeartbeat, NodeID: uint32(i * 10)}); err != nil {
			t.Fatal(err)
		}
	}

	var got []DeltaEntry
	n, err := w.Replay(func(e DeltaEntry) error {
		got = append(got, e)
		return nil
	})
	if err != nil {
...
func TestReplayFnErrorStopsAndReturns(t *testing.T) {
	...
	_ = w.Append(DeltaEntry{SeqNo: 1})
	_ = w.Append(DeltaEntry{SeqNo: 2})

	count := 0
	_, err := w.Replay(func(DeltaEntry) error {
		count++
...
func TestReplaySkipsCorruptJSONEntry(t *testing.T) {
	...
	good1, _ := json.Marshal(DeltaEntry{SeqNo: 1, Type: DeltaRegister})
	bad := []byte("{not valid json")
	good2, _ := json.Marshal(DeltaEntry{SeqNo: 9, Type: DeltaHeartbeat})
...
func TestReplayRejectsOversizedEntry(t *testing.T) {
	...
	_, err := w.Replay(func(DeltaEntry) error { return nil })
	if err == nil {
...
func TestReplayLeavesFileAtEndForAppend(t *testing.T) {
	...
	if _, err := w.Replay(func(DeltaEntry) error { return nil }); err != nil {
		t.Fatal(err)
	}

	// A subsequent Append must land at the end, not overwrite existing bytes.
	if err := w.Append(DeltaEntry{SeqNo: 3}); err != nil {
		t.Fatal(err)
	}
	fi, _ := os.Stat(p)
	if fi.Size() <= before {
...
func TestTruncateThenAppendYieldsSingleEntry(t *testing.T) {
	...
	_ = w.Append(DeltaEntry{SeqNo: 1})
	_ = w.Truncate()
	if err := w.Append(DeltaEntry{SeqNo: 99, Type: DeltaTags}); err != nil {
		t.Fatal(err)
	}
