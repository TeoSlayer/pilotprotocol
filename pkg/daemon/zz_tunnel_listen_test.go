...
func TestListenBindsUDPSocketAndSetsConn(t *testing.T) {
	...
	defer tm.Close()
	if tm.sock == nil {
		t.Fatalf("tm.sock nil after Listen")
	}
	if tm.LocalAddr() == nil {
...
func TestListenResolveErrorReturnsResolveWrappedError(t *testing.T) {
	...
	if tm.sock != nil {
		t.Fatalf("tm.sock should be nil on resolve error")
	}
}
...
func TestListenBindErrorReturnsListenWrappedError(t *testing.T) {
	...
	if second.sock != nil {
		t.Fatalf("second tm.sock should be nil on bind error")
	}
}
...
func TestCloseWithoutListenJustSignalsDoneAndClosesRecvCh(t *testing.T) {
	t.Parallel()
	tm := NewTunnelManager()
	// No Listen → tm.sock is nil → Close skips sock.Close but still closes recvCh + done.
	if err := tm.Close(); err != nil {
