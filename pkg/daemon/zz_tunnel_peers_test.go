...
func TestRemovePeerDropsBothPeerAndCryptoEntries(t *testing.T) {
	...
	tm.envelope.Install(7, &peerCrypto{})
...
func TestHasCryptoReflectsCryptoMap(t *testing.T) {
	...
	tm.envelope.Install(1, &peerCrypto{})
	if !tm.HasCrypto(1) {
...
func TestIsEncryptedRequiresReadyFlag(t *testing.T) {
	...
	pc := &peerCrypto{Ready: false}
	tm.envelope.Install(1, pc)
	if tm.IsEncrypted(1) {
...
func TestPeerListReportsEndpointEncryptedAuthenticatedRelay(t *testing.T) {
	...
	tm.envelope.Install(20, &peerCrypto{Ready: true, Authenticated: true})
	tm.routing.SetRelayPeer(30, true)
...
func TestAddPeerWithEncryptionTriggersKeyExchange(t *testing.T) {
	...
	defer conn.Close()
	tm.sock = udpio.WrapConn(conn)
	tm.routing.SetSocket(tm.sock)
