...
func TestKeepaliveAbsentForIdlePeer(t *testing.T) {
	...
	last, ok := tm.routing.LastOutboundSend(peerNodeID)
...
func TestHandleEncryptedDropsKeepaliveBeforeRecvCh(t *testing.T) {
	...
	gotRecv := tm.routing.HasDirectRecv(peerNodeID)
	if !gotRecv {
...
func TestRecordOutboundSendStampsTimestamp(t *testing.T) {
	...
	_, present := tm.routing.LastOutboundSend(peerNodeID)
	if present {
	...
	stamped, ok := tm.routing.LastOutboundSend(peerNodeID)
	if !ok {
