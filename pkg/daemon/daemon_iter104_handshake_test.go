...
func TestHandleConnectionClosedRecvBufReturnsCleanly(t *testing.T) {
	...
	close(conn.RecvBuf) // triggers `ok=false` branch at handshake.go:269

	done := make(chan struct{})
	go func() {
		d.handshakes.handleConnection(conn)
		close(done)
	}()
	select {
	case <-done:
