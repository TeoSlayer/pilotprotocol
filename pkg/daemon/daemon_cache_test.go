...
func TestKnownNetworkSetUnionsFromAllFourSources(t *testing.T) {
	...
	d.policyMu.Lock()
	d.policyRunners[200] = &PolicyRunner{}
	d.policyMu.Unlock()
	...
	set := d.knownNetworkSet()
	for _, netID := range []uint16{100, 200, 300, 400} {
		if !set[netID] {
			t.Fatalf("netID=%d missing from union", netID)
		}
	}
	if len(set) != 4 {
		t.Fatalf("expected 4 entries, got %d: %v", len(set), set)
	}
}
...
func TestKnownNetworkSetDeduplicatesAcrossSources(t *testing.T) {
	...
	d.netPolicyMu.Unlock()
	d.policyMu.Lock()
	d.policyRunners[50] = &PolicyRunner{}
	d.policyMu.Unlock()
	d.memberTagsMu.Lock()
