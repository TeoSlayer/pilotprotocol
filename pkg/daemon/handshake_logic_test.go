...
// newTestHM returns a fresh HandshakeManager wired to a bare Daemon.
// identityPath can be "" to disable persistence.
func newTestHM(t *testing.T, identityPath string) *HandshakeManager {
	t.Helper()
	d := New(Config{IdentityPath: identityPath})
	return NewHandshakeManager(d)
}
...
func TestSignHandshakeChallengeNilIdentityReturnsEmpty(t *testing.T) {
	...
	if hm.daemon.identity != nil {
		t.Fatal("identity should be nil on bare daemon")
	}
