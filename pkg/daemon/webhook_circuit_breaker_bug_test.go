...
// open. After webhookCircuitOpenThreshold (5) consecutive total
// failures, every subsequent event hits the breaker check at the
// top of post() and increments CircuitSkips instead of attempting
// HTTP. The first event after circuitCooldown elapses is the probe;
...
func TestWebhookHasNoCircuitBreakerForDeadURL(t *testing.T) {
	...
	maxExpected := uint32(webhookCircuitOpenThreshold*webhookMaxRetries + webhookMaxRetries)
	if got > maxExpected {
		t.Errorf("circuit breaker did not curb POSTs: server got %d, expected <= %d (open=%d × max_retries=%d + slack=%d)",
			got, maxExpected, webhookCircuitOpenThreshold, webhookMaxRetries, webhookMaxRetries)
	}
	if skips == 0 {
		t.Errorf("expected CircuitSkips > 0 after breaker opened; got 0")
	}
	if int(skips)+int(got/webhookMaxRetries) < numEvents-2 {
		t.Errorf("skips (%d) + posts/retries (%d) does not account for ~%d events emitted",
			skips, got/webhookMaxRetries, numEvents)
	}
}
...
func TestWebhookCircuitClosesOnSuccessAfterCooldown(t *testing.T) {
	...
	for i := 0; i < webhookCircuitOpenThreshold+1; i++ {
		wc.Emit("phase1", i)
	}
	...
	if wc.circuitOpenUntilNano.Load() == 0 {
		t.Fatalf("expected breaker open after %d failures", webhookCircuitOpenThreshold+1)
	}
