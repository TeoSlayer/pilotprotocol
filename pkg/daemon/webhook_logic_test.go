...
func TestWithHTTPTimeoutSetsClientTimeout(t *testing.T) {
	wc := NewWebhookClient("http://example.invalid", func() uint32 { return 1 },
...
func TestNewWebhookClientEmptyURLReturnsNil(t *testing.T) {
	if NewWebhookClient("", func() uint32 { return 1 }) != nil {
...
func TestEmitAfterCloseIsNoOp(t *testing.T) {
	...
	wc := NewWebhookClient(srv.URL, func() uint32 { return 42 })
	wc.Close()
...
func TestEmitDeliversToServerAndIncrementsEventID(t *testing.T) {
	...
	wc := NewWebhookClient(srv.URL, func() uint32 { return 0xABCD },
...
func TestPost5xxRetriesUpToMaxRetries(t *testing.T) {
	...
	if got := calls.Load(); got != webhookMaxRetries {
		t.Fatalf("calls = %d, want %d (5xx should retry to max)", got, webhookMaxRetries)
	}
}
...
func TestPostNetworkErrorRetries(t *testing.T) {
	...
	wc := NewWebhookClient(url, func() uint32 { return 1 },
		WithRetryBackoff(1*time.Millisecond),
		WithHTTPTimeout(50*time.Millisecond))
	wc.Emit("deadport", nil)
	// Network errors retry until webhookMaxRetries, backoff is tiny (1ms → 2ms → 4ms).
