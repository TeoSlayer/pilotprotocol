...
func TestWriteFrameFlipsToRelayOnTransientReceiveGap(t *testing.T) {
	...
	d.tunnels.routing.RecordDirectRecv(peerNode, time.Now().Add(-9*time.Second))
	// Sanity: not already in relay mode.
	if d.tunnels.routing.IsRelayPeer(peerNode) {
	...
	flipped := d.tunnels.routing.IsRelayPeer(peerNode)
	misses := d.tunnels.routing.BlackholeMissCount(peerNode)
	if flipped {
...
// TestRelayPinnedNeverAutoClear verifies that registry-pinned and
// beacon-admitted symmetric-NAT relay peers are never auto-cleared by
// direct-path observations. Only an explicit SetRelayPeerPinned(id, false)
// can move them back to direct.
//
// Unpinned relay peers (dial-timeout fallback) can auto-clear after
// routing.DirectClearsRequired consecutive direct receipts — see
// TestRelay_UnpinnedClearsAfterDirectThreshold in the routing package.
func TestRelayPinnedNeverAutoClear(t *testing.T) {
	t.Parallel()
	d := New(Config{})
	t.Cleanup(func() { d.tunnels.Close() })
	...
	// Authoritative pin (registry relay_only=true or beacon-admitted symmetric NAT).
	d.tunnels.routing.SetRelayPeerPinned(peerNode, true)

	// Any number of direct receipts must NOT clear a pinned peer.
	for i := 0; i < 50; i++ {
		cleared := d.tunnels.clearRelayOnDirectLocked(peerNode, peerAddr)
		if cleared {
			t.Fatalf("pinned relay must never auto-clear, but cleared=true on receipt #%d", i+1)
		}
	}
	if !d.tunnels.routing.IsRelayPeer(peerNode) {
		t.Fatal("relay flag must remain set after 50 direct receipts on a pinned peer")
	}

	// Explicit unpin must still work.
	d.tunnels.routing.SetRelayPeerPinned(peerNode, false)
	if d.tunnels.routing.IsRelayPeer(peerNode) {
		t.Fatal("SetRelayPeerPinned(false) must clear both relay flag and pin")
	}
}
