...
// Wire format (issue #99): server-pushed CmdRecv / CmdCloseOK envelopes
// look like `[cmd(1)][reqID(8)=0][connID(4)][data...]`. Body offset is
// IPCEnvelopeHeaderSize (9), and the cmd-specific header (4-byte connID)
// sits at offsets [9:13], followed by data.
const recvBodyOff = IPCEnvelopeHeaderSize + 4

// --- data path: RecvBuf → CmdRecv frame with [CmdRecv][reqID(8)=0][connID(4)][data] ---
...
func TestStartRecvPusherForwardsDataAsCmdRecv(t *testing.T) {
	...
	if id := binary.BigEndian.Uint32(reply[IPCEnvelopeHeaderSize : IPCEnvelopeHeaderSize+4]); id != 0xDEADBEEF {
		t.Fatalf("conn id = 0x%X, want 0xDEADBEEF", id)
	}
	if string(reply[recvBodyOff:]) != "hello" {
		t.Fatalf("payload = %q, want 'hello'", string(reply[recvBodyOff:]))
	}
}
...
func TestStartRecvPusherForwardsMultipleChunksInOrder(t *testing.T) {
	...
	for _, want := range []string{"one", "two", "three"} {
		...
		if string(reply[recvBodyOff:]) != want {
			t.Fatalf("payload = %q, want %q", string(reply[recvBodyOff:]), want)
		}
	}
}
...
func TestStartRecvPusherEmitsCmdCloseOKOnRecvBufClose(t *testing.T) {
	...
	// Second frame: close notification — `[cmd(1)][reqID(8)=0][connID(4)]`
	...
	if len(reply2) != IPCEnvelopeHeaderSize+4 {
		t.Fatalf("close frame len = %d, want %d", len(reply2), IPCEnvelopeHeaderSize+4)
	}
	if id := binary.BigEndian.Uint32(reply2[IPCEnvelopeHeaderSize : IPCEnvelopeHeaderSize+4]); id != 7 {
...
func TestStartRecvPusherForwardsEmptyChunk(t *testing.T) {
	...
	if len(reply) != IPCEnvelopeHeaderSize+4 {
		t.Fatalf("empty-chunk frame len = %d, want %d (envelope header + connID only)", len(reply), IPCEnvelopeHeaderSize+4)
	}
	if reply[0] != CmdRecv {
		t.Fatalf("reply[0] = 0x%02X, want CmdRecv", reply[0])
	}
	if id := binary.BigEndian.Uint32(reply[IPCEnvelopeHeaderSize : IPCEnvelopeHeaderSize+4]); id != 123 {
