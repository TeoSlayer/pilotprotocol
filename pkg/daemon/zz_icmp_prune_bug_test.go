...
func TestICMPUnreachableIgnoredByWriteFrame(t *testing.T) {
	...
	count := tm.routing.SendErrCount(peerNodeID)
	flipped := tm.routing.IsRelayPeer(peerNodeID)
	if count != 1 || flipped {
...
func TestRecordInboundDecryptClearsSendErrCount(t *testing.T) {
	...
	// Pre-seed by triggering 2 ICMP errors via handleSendError (under threshold)
	icmpErr := &net.OpError{Op: "write", Net: "udp", Err: syscall.ECONNREFUSED}
	tm.handleSendError(peerNodeID, icmpErr)
	tm.handleSendError(peerNodeID, icmpErr)
	...
	if c := tm.routing.SendErrCount(peerNodeID); c != 0 {
		t.Errorf("expected sendErrCount cleared after inbound decrypt; got %d", c)
	}
}
...
func TestSendErrCountClearedOnPeerRemoval(t *testing.T) {
	...
	tm.peers[peerNodeID] = &net.UDPAddr{IP: net.ParseIP("127.0.0.1"), Port: 1}
	tm.mu.Unlock()
	// Seed sendErrCount via handleSendError calls (no direct map access).
	icmpErr := &net.OpError{Op: "write", Net: "udp", Err: syscall.ECONNREFUSED}
	for i := 0; i < 2; i++ {
		tm.handleSendError(peerNodeID, icmpErr)
	}
	...
	if c := tm.routing.SendErrCount(peerNodeID); c != 0 {
		t.Errorf("expected sendErrCount cleared after RemovePeer; got %d", c)
	}
}
