...
// --- startDataExchangeService: binds + exits on stopCh ---

func TestStartDataExchangeServiceBindsAndExitsOnStop(t *testing.T) {
	d := New(Config{})
	if err := d.startDataExchangeService(); err != nil {
		t.Fatalf("startDataExchangeService: %v", err)
	}
	if _, err := d.ports.Bind(protocol.PortDataExchange); err == nil {
		t.Fatal("PortDataExchange should already be bound")
	}
	close(d.stopCh)
	time.Sleep(100 * time.Millisecond)
}

// --- startDataExchangeService: error path — port already bound ---

func TestStartDataExchangeServiceErrorsWhenPortAlreadyBound(t *testing.T) {
	d := New(Config{})
	if _, err := d.ports.Bind(protocol.PortDataExchange); err != nil {
		t.Fatalf("pre-bind: %v", err)
	}
	defer d.ports.Unbind(protocol.PortDataExchange)
	if err := d.startDataExchangeService(); err == nil {
		t.Fatal("startDataExchangeService should error when port already bound")
	}
}

// --- startEventStreamService: binds + exits on stopCh ---

func TestStartEventStreamServiceBindsAndExitsOnStop(t *testing.T) {
	d := New(Config{})
	if err := d.startEventStreamService(); err != nil {
		t.Fatalf("startEventStreamService: %v", err)
	}
	if _, err := d.ports.Bind(protocol.PortEventStream); err == nil {
		t.Fatal("PortEventStream should already be bound")
	}
	close(d.stopCh)
	time.Sleep(100 * time.Millisecond)
}
...
func TestStartBuiltinServicesBindsAllEnabledByDefault(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("HOME", tmp) // tasksubmit may write tasks dir
...
// --- startBuiltinServices: all four disable flags → no ports bound ---
