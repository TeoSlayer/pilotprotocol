...
func TestHandleBindAcceptGoroutineEmitsCmdAccept(t *testing.T) {
	...
	go func() {
		s.handleBind(ic, []byte{0x08, 0x00}) // port 2048
		close(done)
	}()
	...
	// Expect CmdAccept frame: [CmdAccept][port(2)][connID(4)][addr(6)][remotePort(2)]
	_ = client.SetReadDeadline(time.Now().Add(2 * time.Second))
	acceptReply, err := ipcutil.Read(client)
	if err != nil {
		t.Fatalf("read accept frame: %v", err)
	}
	want := 1 + 2 + 4 + protocol.AddrSize + 2
	...
	if p := binary.BigEndian.Uint16(acceptReply[1:3]); p != 2048 {
		t.Fatalf("accept.port = %d, want 2048", p)
	}
	if id := binary.BigEndian.Uint32(acceptReply[3:7]); id != 0xABCD {
		t.Fatalf("accept.connID = 0x%X, want 0xABCD", id)
	}
	gotAddr := protocol.UnmarshalAddr(acceptReply[7 : 7+protocol.AddrSize])
	if gotAddr.Network != 1 || gotAddr.Node != 0x10203040 {
		t.Fatalf("accept.addr = %+v, want {Network:1 Node:0x10203040}", gotAddr)
	}
	if rp := binary.BigEndian.Uint16(acceptReply[7+protocol.AddrSize:]); rp != 5555 {
...
func TestHandleBindPortAlreadyBoundReturnsError(t *testing.T) {
	...
	reply := runHandler(t, client, func() {
		s.handleBind(ic, []byte{0x05, 0xDC}) // port 1500
	})
...
func TestHandleSendShortPayloadEmitsError(t *testing.T) {
	...
	ic, client := newIPCTestConn(t)
	reply := runHandler(t, client, func() {
		s.handleSend(ic, []byte{0x01, 0x02}) // only 2 bytes — need ≥4
	})
...
func TestHandleCloseShortPayloadEmitsError(t *testing.T) {
	...
	ic, client := newIPCTestConn(t)
	reply := runHandler(t, client, func() {
		s.handleClose(ic, []byte{0x01}) // 1 byte < 4
	})
...
func TestDeliverDatagramWritesCmdRecvFromToClient(t *testing.T) {
	...
	// Frame layout: [CmdRecvFrom][addr(6)][srcPort(2)][dstPort(2)][data...]
	wantLen := 1 + protocol.AddrSize + 2 + 2 + len(payload)
	...
	gotAddr := protocol.UnmarshalAddr(reply[1 : 1+protocol.AddrSize])
	if gotAddr != srcAddr {
		t.Fatalf("srcAddr = %+v, want %+v", gotAddr, srcAddr)
	}
	if sp := binary.BigEndian.Uint16(reply[1+protocol.AddrSize:]); sp != 443 {
		t.Fatalf("srcPort = %d, want 443", sp)
	}
	if dp := binary.BigEndian.Uint16(reply[1+protocol.AddrSize+2:]); dp != 8080 {
		t.Fatalf("dstPort = %d, want 8080", dp)
	}
	if string(reply[1+protocol.AddrSize+4:]) != "udp-data" {
		t.Fatalf("payload = %q, want 'udp-data'", string(reply[1+protocol.AddrSize+4:]))
	}
}
