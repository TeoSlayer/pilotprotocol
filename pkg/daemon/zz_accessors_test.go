...
// Daemon-side wrapper tests. The PolicyRunner type itself is in
// plugins/policy as part of T2.3; the daemon-side wrappers delegate to
// the registered coreapi.PolicyManager. With no manager registered
// these tests assert the nil-manager branches.

func TestGetPolicyRunnerNilWhenAbsent(t *testing.T) {
	t.Parallel()
	d := New(Config{})
	if pr := d.GetPolicyRunner(200); pr != nil {
		t.Fatalf("GetPolicyRunner(200) = %v, want nil (no policy manager registered)", pr)
	}
}

func TestStartPolicyRunnerWithoutManagerReturnsError(t *testing.T) {
	t.Parallel()
	d := New(Config{})
	err := d.StartPolicyRunner(201, json.RawMessage("{not json"))
	if err == nil {
		t.Fatalf("expected error when policy manager is not registered")
	}
}
...
func TestStopPolicyRunnerMissingIsNoop(t *testing.T) {
	t.Parallel()
	d := New(Config{})
	d.StopPolicyRunner(0xFFFF) // should not panic with nil manager
}
...
// stopPolicyRunners is now a thin delegate to the registered policy
// manager. With no manager (default test daemon) it's a no-op.
// Multi-runner stop behavior is exercised in plugins/policy where
// the manager lives.
