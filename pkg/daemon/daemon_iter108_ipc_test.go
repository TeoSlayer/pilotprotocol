...
func TestIPCServerAcceptLoopDispatchesCommand(t *testing.T) {
	...
	if err := ipcutil.Write(conn, []byte{CmdBind, 0x10, 0xE1}); err != nil {
...
func TestIPCServerHandleClientUnknownCmdSendsError(t *testing.T) {
	...
	if err := ipcutil.Write(conn, []byte{0xFE}); err != nil {
...
func TestIPCServerHandleClientEmptyFrameContinuesLoop(t *testing.T) {
	...
	// Send a zero-length frame, then a valid CmdBind. If the continue
	// branch works, the second frame still produces a CmdBindOK.
	if err := ipcutil.Write(conn, []byte{}); err != nil {
		t.Fatalf("write empty frame: %v", err)
	}
	if err := ipcutil.Write(conn, []byte{CmdBind, 0x20, 0x01}); err != nil {
		t.Fatalf("write bind: %v", err)
	}
	_ = conn.SetReadDeadline(time.Now().Add(2 * time.Second))
	reply, err := ipcutil.Read(conn)
	if err != nil {
		t.Fatalf("read bind reply after empty: %v", err)
	}
	if reply[0] != CmdBindOK {
		t.Fatalf("reply[0] = 0x%02X, want CmdBindOK (empty-frame branch skipped it)", reply[0])
	}
}
...
func TestIPCServerHandleClientCleansUpTrackedPorts(t *testing.T) {
	...
	if err := ipcutil.Write(conn, []byte{CmdBind, 0x04, 0xD2}); err != nil {
		t.Fatalf("write bind: %v", err)
	}
	_ = conn.SetReadDeadline(time.Now().Add(2 * time.Second))
	reply, err := ipcutil.Read(conn)
