...
func TestAtomicWritePermissions0600(t *testing.T) {
	...
	if err := fsutil.AtomicWrite(path, []byte("secret")); err != nil {
		t.Fatalf("AtomicWrite: %v", err)
	}
...
func TestAppendSyncCreatesFile(t *testing.T) {
	...
	if err := fsutil.AppendSync(path, []byte("line1\n")); err != nil {
...
func TestAppendSyncAppends(t *testing.T) {
	...
	if err := fsutil.AppendSync(path, []byte("a\n")); err != nil {
		t.Fatalf("first AppendSync: %v", err)
	}
	if err := fsutil.AppendSync(path, []byte("b\n")); err != nil {
...
func TestAppendSyncPermissions0600(t *testing.T) {
	...
	if err := fsutil.AppendSync(path, []byte("x")); err != nil {
		t.Fatalf("AppendSync: %v", err)
	}
...
func TestAppendSyncBadDirectory(t *testing.T) {
	t.Parallel()
	err := fsutil.AppendSync("/nonexistent/dir/file.log", []byte("data"))
	if err == nil {
		t.Fatal("expected error for nonexistent directory")
	}
}
