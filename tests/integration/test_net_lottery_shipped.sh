...
log_test "repeated connects yield non-deterministic pass/fail"
PASS=0; DENY=0
for i in $(seq 1 10); do
    if echo lot-$i | $DC exec -T agent-a pilotctl connect agent-b 7 --timeout 2s >/dev/null 2>&1; then
        PASS=$((PASS+1))
    else
        DENY=$((DENY+1))
    fi
done
if [ "$PASS" -gt 0 ] && [ "$DENY" -gt 0 ]; then
    log_pass "lottery outcome mixed: pass=$PASS deny=$DENY"
else
    log_fail "deterministic outcome pass=$PASS deny=$DENY (EXPECTED: probabilistic)"
fi
