...
sleep 1
SC_PRE=$($DC exec -T agent-a pilotctl --json managed score "$NID" "$PEER_B" 2>/dev/null | jq -r ".data.score // 0")
$DC exec -T agent-a pilotctl --json managed cycle --force --net "$NID" >/dev/null 2>&1
SC_POST=$($DC exec -T agent-a pilotctl --json managed score "$NID" "$PEER_B" 2>/dev/null | jq -r ".data.score // 0")
if [ "${SC_POST:-0}" -lt "${SC_PRE:-0}" ]; then
