...
$DC exec -T agent-a bash -c '
    ...
    # Wait up to 10s for registrations
    for j in $(seq 1 20); do
        REG=$(curl -fsS http://172.29.0.10:8080/api/stats 2>/dev/null | jq -r ".total_nodes // 0")
        if [ "$REG" -ge 50 ]; then break; fi
        sleep 0.5
    done
...
# Allow up to 5 successes (timing slack — polo may not have been ranked
# at exact moment of submit). Real defense should be >= 45 rejections.
if [ "$REJECT" -ge 45 ]; then
    log_pass "polo-gate rejected $REJECT / 50 sybil submits"
else
    log_fail "only $REJECT / 50 rejected — polo-gate weak or absent"
fi
