...
# ----- 2. Submit tasks during the split ---------
# We don't expect these to complete before heal. Record ids.
log_test "submit 3 tasks during partition (not expected to complete yet)"
TIDS=()
for i in 1 2 3; do
    S=$($DC exec -T agent-a bash -c "timeout 8 pilotctl --json task submit agent-b --task 'split-$i'" 2>&1)
    T=$(echo "$S" | jq -r '.data.task_id // empty')
    [ -n "$T" ] && TIDS+=("$T")
done
log_pass "submitted ${#TIDS[@]} task ids during split"
...
for _ in $(seq 1 30); do
    if $DC exec -T agent-a timeout 3 ping -c 1 -W 2 "$B_IP" >/dev/null 2>&1; then OK=yes; break; fi
    sleep 1
done
if [ "$OK" = "yes" ]; then
    log_pass "L3 path restored"
else
