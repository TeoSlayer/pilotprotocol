...
PID=$($DC exec -T agent-b pilotctl --json pending 2>/dev/null | jq -r '.data[0].peer_id // empty' 2>/dev/null)
if [ -n "$PID" ] && [ "$PID" != "null" ]; then
    $DC exec -T agent-b pilotctl approve "$PID" >/dev/null 2>&1 || true
fi
sleep 5

GRANT=$($DC exec -T webhook-sink sh -c "grep -cE '\"event\":\"(trust\\.changed|handshake\\.approved|handshake\\.auto_approved)\"' $LOG 2>/dev/null || echo 0")
...
BEFORE_REV=$($DC exec -T webhook-sink sh -c "grep -cE '\"event\":\"(trust\\.changed|trust\\.revoked|trust\\.revoked_by_peer)\"' $LOG 2>/dev/null || echo 0")
$DC exec -T agent-a pilotctl untrust agent-b >/dev/null 2>&1 || true
sleep 5
AFTER_REV=$($DC exec -T webhook-sink sh -c "grep -cE '\"event\":\"(trust\\.changed|trust\\.revoked|trust\\.revoked_by_peer)\"' $LOG 2>/dev/null || echo 0")
...
CANON=$($DC exec -T webhook-sink sh -c "grep -c '\"event\":\"trust.changed\"' $LOG 2>/dev/null || echo 0")
if [ "$CANON" -ge 1 ]; then
