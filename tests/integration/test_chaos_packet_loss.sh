...
S2=$($DC exec -T agent-a pilotctl --json task submit agent-b --task "chaos-30pct" 2>&1)
TID1=$(echo "$S2" | jq -r '.data.task_id // empty')
if [ -z "$TID1" ]; then
    log_fail "submit under chaos failed: $(echo "$S2" | head -c 300)"
else
    LOSSY=""
    for _ in $(seq 1 60); do
    ...
    if echo "$LOSSY" | grep -qiE "completed|succeeded|done"; then
        ...
        log_fail "task stuck under 30% loss (status=$LOSSY) — retransmit gap"
    fi
fi
...
if echo "$POST" | grep -qiE "completed|succeeded|done"; then
    ...
    log_fail "post-chaos task stuck (status=$POST)"
fi
