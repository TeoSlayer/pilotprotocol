#!/bin/bash
# Shipped config: configs/networks/last-in-first-out.json
#
# Name's promise: LIFO eviction — newest joiner is evicted first when cap hit
#
...
NID=$(create_network_from_file "$CFG" "last-in-first-out-$$" "1m") || { log_fail "create"; exit 1; }
log_pass "net=$NID"
...
if echo "$STATUS" | grep -qE "joined_at|joinedAt|order"; then
    ...
    log_fail "no joined-order surface (EXPECTED: LIFO ordering visible)"
fi
