...
# After agent-b restarts, b's tunnel state is fresh while a's is stale.
# Drive a ping b->a so b sends an auth key exchange, a installs new
# crypto, and the subsequent send-results frame can decrypt cleanly.
log_test "send-results after restart (rekey via warm-up ping)"
$DC exec -T agent-b pilotctl ping agent-a --count 2 --timeout 5s >/dev/null 2>&1 || true
