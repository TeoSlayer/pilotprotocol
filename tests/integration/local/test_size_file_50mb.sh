#!/bin/bash
# Large payload: 50 MiB send-file with SHA-256 integrity verification.
# DURATION: 5min
...
log_test "send file to agent-b (generous timeout)"
OUT=$($DC exec -T agent-a bash -c "pilotctl --json send-file agent-b /tmp/size-${SIZE_MB}m.bin --timeout 300s 2>&1")
