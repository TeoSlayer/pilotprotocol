...
BEFORE=$($DC exec -T webhook-sink sh -c "grep -c '\"event\":\"polo.updated\"' $LOG 2>/dev/null; true" | tail -1)
...
AFTER=$($DC exec -T webhook-sink sh -c "grep -c '\"event\":\"polo.updated\"' $LOG 2>/dev/null; true" | tail -1)
DELTA=$((AFTER - BEFORE))
