...
OUT=$($DC exec -T agent-b bash -c "pilotctl --json task send-results --id $TID --file /tmp/bigresult.txt --timeout 300s 2>&1")
OK=$(echo "$OUT" | jq -r ".status // empty")
if [ "$OK" = "ok" ]; then
    log_pass "send-results ok"
else
    log_fail "send-results failed: $(echo "$OUT" | head -c 300)"
fi
