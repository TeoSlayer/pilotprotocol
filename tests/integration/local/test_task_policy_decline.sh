...
terminal() {
    echo "$1" | grep -qiE "completed|succeeded|done|declined|cancelled"
}
...
# ----- 5. decline-* tasks are DECLINED/CANCELLED -----
log_test "decline-beta and decline-delta are DECLINED (or CANCELLED)"
is_declined() { echo "$1" | grep -qE "^(DECLINED|CANCELLED)$"; }
if is_declined "$ST_B" && is_declined "$ST_D"; then
    log_pass "both decline-* tasks declined (beta=$ST_B delta=$ST_D)"
else
...
ACC_CNT=$($DC exec -T agent-b bash -c "grep -c 'accept policy' /tmp/worker.log 2>/dev/null || true")
DEC_CNT=$($DC exec -T agent-b bash -c "grep -c 'decline policy' /tmp/worker.log 2>/dev/null || true")
