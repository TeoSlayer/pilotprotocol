...
# Default target = a public production agent. Override via env so the
# suite can point at a local stack (e.g., docker compose's agent-b)
# instead of depending on an external node being reachable from CI.
ALPHA_AGENT="${PILOT_CLI_TARGET_AGENT:-agent-alpha}"
