...
sleep 2
sync_policy_peers "$NID_X" agent-a agent-b
sync_policy_peers "$NID_Y" agent-a agent-b
...
# `on: connect` fires on RECEIVER — read agent-b's view of PEER_A.
log_test "score in X is >0 (X's rule awarded +5/connect)"
SC_X=$(peer_score agent-b "$NID_X" "$PEER_A")
if [ "${SC_X:-0}" -gt 0 ]; then
...
SC_Y=$(peer_score agent-b "$NID_Y" "$PEER_A")
if [ -z "$SC_Y" ] || [ "${SC_Y:-0}" = "0" ]; then
    log_pass "Y score = ${SC_Y:-0} (polo ledger is per-network)"
else
