...
RESP_CNT=$($DC exec -T agent-a bash -c "grep -c 'respond req=' /tmp/responder.log 2>/dev/null || true")
if [ "$RESP_CNT" = "2" ]; then
...
MUL_CNT=$($DC exec -T agent-b bash -c "grep -c 'mul=$MULTIPLIER' /tmp/worker.log 2>/dev/null || true")
if [ "$MUL_CNT" = "2" ]; then
