...
S=$($DC exec -T agent-a pilotctl --json task submit agent-b --task "loss30-task" 2>&1)
TID=$(echo "$S" | jq -r '.data.task_id // empty')
if [ -z "$TID" ]; then
    log_fail "submit did not return id"
else
    STA=""
    ...
    if echo "$STA" | grep -qiE "completed|succeeded|done"; then
        ...
        log_fail "task stuck (status=$STA) — expected P1-010 finding"
    fi
fi
...
# ----- register -----
log_test "register under 30% loss"
if $DC exec -T agent-a timeout 20 pilotctl register >/dev/null 2>&1; then
    log_pass "register ok"
else
    log_fail "register failed"
fi
