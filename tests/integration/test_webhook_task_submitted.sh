...
BEFORE=$($DC exec -T webhook-sink sh -c "grep -c '\"event\":\"task.submitted\"' $LOG 2>/dev/null || echo 0")
...
AFTER=$($DC exec -T webhook-sink sh -c "grep -c '\"event\":\"task.submitted\"' $LOG 2>/dev/null || echo 0")
DELTA=$((AFTER - BEFORE))
