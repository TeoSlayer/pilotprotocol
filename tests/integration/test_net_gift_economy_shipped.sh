...
echo gift | $DC exec -T agent-a pilotctl connect agent-b 7 --timeout 3s >/dev/null 2>&1 || true
sleep 1
SC_B=$($DC exec -T agent-a pilotctl --json managed score "$NID" "$PEER_B" 2>/dev/null | jq -r ".data.score // 0")
SC_A=$($DC exec -T agent-b pilotctl --json managed score "$NID" "$PEER_A" 2>/dev/null | jq -r ".data.score // 0")
