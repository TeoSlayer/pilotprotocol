...
# Generate 1 connect then go silent — b becomes the camper.
echo hello | $DC exec -T agent-a pilotctl connect agent-b 7 --timeout 5s >/dev/null 2>&1 || true
PEER_B=$($DC exec -T agent-b pilotctl --json info 2>/dev/null | jq -r '.data.node_id // 0')

log_test "force cycles — expect b evicted after threshold"
for i in 1 2 3 4 5; do
    $DC exec -T agent-a pilotctl --json managed cycle --force --net "$NID" >/dev/null 2>&1 || true
    sleep 1
done

RANKING=$($DC exec -T agent-a pilotctl --json managed rankings "$NID" 2>/dev/null)
if echo "$RANKING" | grep -q "$PEER_B"; then
    # EXPECTED: after idle cycles, b is evicted from the rankings
    # list. If it's still there, anti-camping is not enforced.
    log_fail "b still in rankings after idle cycles (EXPECTED: evicted)"
else
    log_pass "b evicted from rankings after idle cycles"
fi
