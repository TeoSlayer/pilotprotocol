...
# Figure out the image tag compose just built.
...
if [ -z "$IMG" ] || [ "$IMG" = "null:null" ]; then
    # Fallback: grab by inspecting the rendezvous-1 container.
    IMG=$(docker inspect -f '{{.Config.Image}}' $($DC ps -q rendezvous-1 | head -n1) 2>/dev/null)
fi

docker run -d --rm --name "healed-agent-c" --hostname agent-c \
...
docker run -d --rm --name "healed-agent-d" --hostname agent-d \
...
if [ "$MERGED" = "1" ]; then
    ...
    docker logs healed-agent-c 2>&1 | tail -20 | sed 's/^/  c: /'
    docker rm -f healed-agent-c healed-agent-d >/dev/null 2>&1
    $DC down -v >/dev/null 2>&1
    exit 1
fi
...
docker exec -d healed-agent-c bash -c '
...
BAD2=$(docker logs healed-agent-c 2>&1 | grep -iE "panic|fatal|race detected" | head -3)
BAD3=$(docker logs healed-agent-d 2>&1 | grep -iE "panic|fatal|race detected" | head -3)
...
docker exec healed-agent-c touch /tmp/worker_stop >/dev/null 2>&1 || true
docker rm -f healed-agent-c healed-agent-d >/dev/null 2>&1 || true
$DC down -v >/dev/null 2>&1
