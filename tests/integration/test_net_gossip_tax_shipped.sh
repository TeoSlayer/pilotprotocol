...
log_test "excessive pubsub publishes drain score"
PEER_A=$($DC exec -T agent-a pilotctl --json info 2>/dev/null | jq -r ".data.node_id // 0")
SC0=$($DC exec -T agent-b pilotctl --json managed score "$NID" "$PEER_A" 2>/dev/null | jq -r ".data.score // 0")
for i in $(seq 1 15); do
    $DC exec -T agent-a pilotctl publish agent-b t/$i --data gossip-$i --timeout 2s >/dev/null 2>&1 || true
done
sleep 1
SC1=$($DC exec -T agent-b pilotctl --json managed score "$NID" "$PEER_A" 2>/dev/null | jq -r ".data.score // 0")
if [ "${SC1:-0}" -lt "${SC0:-0}" ] || [ "${SC1:-0}" -le 0 ]; then
    log_pass "gossip tax applied: $SC0 -> $SC1"
else
    log_fail "no gossip tax: $SC0 -> $SC1 (EXPECTED: score decrease on high emit)"
fi
