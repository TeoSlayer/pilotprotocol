...
# ----- 8. registry re-register (force a control-plane op) -----
log_test "pilotctl register (explicit re-register) under 10% loss"
RG=$($DC exec -T agent-a timeout 15 pilotctl register 2>&1)
if [ $? -eq 0 ]; then
    log_pass "re-register ok"
else
    log_fail "re-register failed: $(echo "$RG" | head -c 300)"
fi
