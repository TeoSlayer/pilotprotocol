...
$DC exec -T agent-b pilotctl send agent-a 9999 --data grudge --timeout 2s >/dev/null 2>&1 || true
sleep 1
$DC restart agent-a >/dev/null 2>&1
sleep 5
ensure_stack_up || true
# Retry after restart — grudge should persist.
if echo retry | $DC exec -T agent-b pilotctl connect agent-a 7 --timeout 3s >/dev/null 2>&1; then
