...
func TestChunkedReap(t *testing.T) {
	...
	rc, err := registryclient.Dial(resolveLocalAddr(reg.Addr()))
	if err != nil {
		t.Fatalf("dial: %v", err)
	}
	defer rc.Close()
	...
	// Advance past stale threshold (currently 30 min).
...
func TestFlushSaveUnderConcurrentHeartbeat(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			hrc, err := registryclient.Dial(resolveLocalAddr(reg.Addr()))
			if err != nil {
				return
			}
			defer hrc.Close()
	...
	rc2, err := registryclient.Dial(resolveLocalAddr(reg2.Addr()))
	if err != nil {
...
func TestBinaryWireNegotiation(t *testing.T) {
	...
	jsonRC, err := registryclient.Dial(addr)
	if err != nil {
	...
	brc, err := registryclient.DialBinary(addr)
	if err != nil {
...
func TestBinaryAndJSONClientsCoexist(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			jrc, err := registryclient.Dial(addr)
			if err != nil {
...
func TestBinaryLookupNodeNotFound(t *testing.T) {
	...
	brc, err := registryclient.DialBinary(resolveLocalAddr(reg.Addr()))
	if err != nil {
		t.Fatalf("binary dial: %v", err)
	}
	defer brc.Close()
