...
func TestRegistryReplication(t *testing.T) {
	...
	rc, err := registryclient.Dial(primaryAddr)
	if err != nil {
	...
	sc, err := registryclient.Dial(standbyAddr)
	if err != nil {
...
func TestRegistryStandbyPromotion(t *testing.T) {
	...
	{
		sc, err := registryclient.Dial(standby.Addr().String())
		if err != nil {
			t.Fatalf("dial standby: %v", err)
		}
		deadline := time.After(5 * time.Second)
	...
	pc, err := registryclient.Dial(promoted.Addr().String())
	if err != nil {
...
func TestFailoverUnderLoad(t *testing.T) {
	...
	sc2, err := registryclient.Dial(standbyAddr)
	if err != nil {
	...
	pc, err := registryclient.Dial(promotedAddr)
	if err != nil {
		t.Fatalf("dial promoted: %v", err)
	}
	defer pc.Close()
...
// RBAC roles, network policy, enterprise flag, admin token, node tags,
// key metadata, and audit log entries.
func TestReplicationEnterpriseData(t *testing.T) {
	...
	if _, err := rc.SetHostnameAdmin(nodeID1, "repl-node1", TestAdminToken); err != nil {
	...
	// Verify hostname replicated
	lookup1s, err := sc.Lookup(nodeID1)
	if err != nil {
		t.Fatalf("standby lookup node1 for hostname: %v", err)
	}
	if hn, _ := lookup1s["hostname"].(string); hn != "repl-node1" {
		t.Errorf("standby: hostname = %q, want repl-node1", hn)
	}
...
func TestReplicationTokenValidation(t *testing.T) {
	...
	wsc, err := registryclient.Dial(wrongStandby.Addr().String())
	if err != nil {
	...
	csc, err := registryclient.Dial(correctStandby.Addr().String())
	if err != nil {
...
// TestListNodesEnrichedFields verifies that list_nodes includes role,
