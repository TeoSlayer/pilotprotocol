...
func TestAdminNodeManagement(t *testing.T) {
	...
	t.Log("all admin node management methods work correctly")
}
...
// TestAuditEnrichedTagsAndPolicy verifies that tags and policy
// audit entries include old and new values.
func TestAuditEnrichedTagsAndPolicy(t *testing.T) {
	t.Parallel()
	rc, _, cleanup := startTestRegistryWithAdmin(t)
	defer cleanup()
...
func TestConcurrentRBACOperations(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			client, err := registryclient.Dial(env.RegistryAddr)
			if err != nil {
				errors[idx] = err
				return
			}
...
func TestPersistenceEnterprisePolicyWithPorts(t *testing.T) {
	...
	rc, err := registryclient.Dial(reg1.Addr().String())
	if err != nil {
		t.Fatalf("dial: %v", err)
	}
...
func TestConcurrentEnterpriseOps(t *testing.T) {
	...
	for i := 3; i < 5; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			c, err := registryclient.Dial(reg.Addr().String())
			if err != nil {
...
func TestBlueprintPersistence(t *testing.T) {
	...
	rc1, err := registryclient.Dial(srv1.Addr().String())
	if err != nil {
		t.Fatalf("dial: %v", err)
	}
	...
	rc2, err := registryclient.Dial(srv2.Addr().String())
	if err != nil {
		t.Fatalf("dial: %v", err)
	}
	defer rc2.Close()
