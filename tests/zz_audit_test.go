...
func TestAuditLogRingBuffer(t *testing.T) {
	...
	// Register a single node, then set hostname 1010 times to exceed
	// the 1000-entry ring buffer without hitting rate limiters.
	nodeID, _ := registerTestNode(t, rc)
	for i := 0; i < 1010; i++ {
		rc.SetHostnameAdmin(nodeID, fmt.Sprintf("host-%d", i), TestAdminToken)
	}
	// Total events: 1 registration + 1010 hostname.changed = 1011 events.
	...
	// All returned entries should be hostname.changed (the registration was evicted)
	for i, e := range entries {
		entry := e.(map[string]interface{})
		if entry["action"] != "hostname.changed" {
			t.Errorf("entry %d: expected hostname.changed, got %s", i, entry["action"])
		}
	}
...
func TestAuditLogPersistence(t *testing.T) {
	...
	rc1, err := registryclient.Dial(reg1.Addr().String())
	if err != nil {
		t.Fatalf("dial: %v", err)
	}

	// Register a node and set hostname to generate audit events
	nodeID, _ := registerTestNode(t, rc1)
	rc1.SetHostnameAdmin(nodeID, "testhost", TestAdminToken)
	...
	if !actions["hostname.changed"] {
		t.Error("missing hostname.changed after restart")
	}
