...
// provided (`<fp>@nodes.pilotprotocol.network`), so starting without an email
// is expected to succeed and produce a node whose Email field carries the
// synthetic address.
func TestDaemonSynthesisesEmailWhenMissing(t *testing.T) {
	t.Parallel()
	env := NewTestEnv(t)
	...
	if err := d.Start(); err != nil {
		t.Fatalf("daemon start without email should now succeed (synthesised), got: %v", err)
	}
	defer d.Stop()

	info := d.Info()
	if info.Email == "" {
		t.Fatal("Email should be auto-synthesised, got empty")
	}
	if !strings.HasSuffix(info.Email, "@nodes.pilotprotocol.network") {
		t.Errorf("synthesised email should end with @nodes.pilotprotocol.network, got %q", info.Email)
	}
}
