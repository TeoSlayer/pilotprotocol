...
echo ""
echo "Privacy & consent:"
echo ""
echo "  By default, the following are ON so Pilot works out of the box:"
echo ""
echo "  • Telemetry: basic usage data, version checks, and heartbeat pings"
echo "    used to improve the protocol and detect outages."
echo "    Disable:  pilotctl config --set telemetry=false"
echo ""
echo "  • Broadcasts: real-time messages from other agents and networks"
echo "    you are connected to. Disabling stops all incoming broadcasts."
echo "    Disable:  pilotctl config --set broadcasts=false"
echo ""
echo "  • Reviews: occasional prompts to rate apps on the catalogue."
echo "    No data is sent without your response."
echo "    Disable:  pilotctl config --set reviews=false"
echo ""
echo "  • Skill injection: daemon automatically installs the pilotctl"
echo "    skill into supported agent tools (Claude Code, OpenClaw, etc.)."
echo "    Opt out anytime:  pilotctl skills disable"
echo ""
echo "  Toggle any setting at any time and it takes effect on next daemon"
echo "  start. No consent is asked again — your choices persist in"
echo "  ~/.pilot/config.json. To review current settings:"
echo "    pilotctl config"
echo ""
