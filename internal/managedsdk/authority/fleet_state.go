...
func (manager *FleetManager) ReportStateSnapshot(ctx context.Context, snapshot FleetStateSnapshot) error {
	persistence, ok := manager.persistence.(FleetStatePersistence)
	if !ok {
		return fmt.Errorf("authority: fleet state persistence is unavailable")
	}
	publicKey, err := manager.trust.IntentKeyAt(snapshot.TenantID, snapshot.AgentID, snapshot.KeyID, time.Unix(snapshot.ObservedAt, 0))
	if err != nil {
		return err
	}
	if err := snapshot.Verify(publicKey, manager.now()); err != nil {
		return err
	}
	if !manager.nodeOperational(snapshot.TenantID, snapshot.AgentID) {
		return fmt.Errorf("authority: fleet state source is not actively enrolled")
	}
	if current, found, err := persistence.FleetStateSnapshot(ctx, snapshot.TenantID, snapshot.AgentID); err != nil {
		return persistenceError("load fleet state snapshot", err)
	} else if found {
		if snapshot.Revision < current.Revision || snapshot.Revision == current.Revision && snapshot.RootHash != current.RootHash {
			return fmt.Errorf("authority: fleet state snapshot revision conflict")
		}
	}
	if err := persistence.SaveFleetStateSnapshot(ctx, snapshot); err != nil {
		return persistenceError("save fleet state snapshot", err)
	}
	return nil
}

func (manager *FleetManager) StateSnapshot(ctx context.Context, tenantID, agentID string) (FleetStateSnapshot, bool, error) {
	persistence, ok := manager.persistence.(FleetStatePersistence)
	if !ok {
		return FleetStateSnapshot{}, false, fmt.Errorf("authority: fleet state persistence is unavailable")
	}
	snapshot, found, err := persistence.FleetStateSnapshot(ctx, tenantID, agentID)
	if err != nil || !found {
		return snapshot, found, err
	}
	publicKey, keyErr := manager.trust.IntentKeyAt(snapshot.TenantID, snapshot.AgentID, snapshot.KeyID, time.Unix(snapshot.ObservedAt, 0))
	if keyErr != nil || snapshot.VerifySignature(publicKey) != nil || snapshot.TenantID != tenantID || snapshot.AgentID != agentID {
		return FleetStateSnapshot{}, false, fmt.Errorf("authority: invalid stored fleet state snapshot")
	}
	return snapshot, true, nil
}

func (manager *FleetManager) StateSnapshots(ctx context.Context, tenantID string) ([]FleetStateSnapshot, error) {
	persistence, ok := manager.persistence.(FleetStatePersistence)
	if !ok {
		return nil, fmt.Errorf("authority: fleet state persistence is unavailable")
	}
	values, err := persistence.ListFleetStateSnapshots(ctx, tenantID)
	if err != nil {
		return nil, persistenceError("list fleet state snapshots", err)
	}
	for _, snapshot := range values {
		publicKey, keyErr := manager.trust.IntentKeyAt(snapshot.TenantID, snapshot.AgentID, snapshot.KeyID, time.Unix(snapshot.ObservedAt, 0))
		if keyErr != nil || snapshot.VerifySignature(publicKey) != nil || snapshot.TenantID != tenantID {
			return nil, fmt.Errorf("authority: invalid stored fleet state snapshot")
		}
	}
	return values, nil
}

func (manager *FleetManager) StateSnapshotHistory(ctx context.Context, tenantID, agentID string, limit int) ([]FleetStateSnapshot, error) {
	persistence, ok := manager.persistence.(FleetStateHistoryPersistence)
	if !ok {
		return nil, fmt.Errorf("authority: fleet state history persistence is unavailable")
	}
	if limit < 1 || limit > 100 {
		return nil, fmt.Errorf("authority: fleet state history limit must be 1-100")
	}
	values, err := persistence.ListFleetStateSnapshotHistory(ctx, tenantID, agentID, limit)
	if err != nil {
		return nil, persistenceError("list fleet state history", err)
	}
	for _, snapshot := range values {
		publicKey, keyErr := manager.trust.IntentKeyAt(snapshot.TenantID, snapshot.AgentID, snapshot.KeyID, time.Unix(snapshot.ObservedAt, 0))
		if keyErr != nil || snapshot.VerifySignature(publicKey) != nil || snapshot.TenantID != tenantID || snapshot.AgentID != agentID {
			return nil, fmt.Errorf("authority: invalid stored fleet state history")
		}
	}
	return values, nil
}

func (manager *FleetManager) QueueStateMutation(ctx context.Context, mutation FleetStateMutation) error {
	persistence, ok := manager.persistence.(FleetStatePersistence)
	if !ok {
		return fmt.Errorf("authority: fleet state persistence is unavailable")
	}
	publicKey, err := manager.trust.DecisionKey(ctx, mutation.TenantID, mutation.KeyID)
	if err != nil {
		return err
	}
	if err := mutation.Verify(publicKey, manager.now()); err != nil {
		return err
	}
	if !manager.nodeOperational(mutation.TenantID, mutation.AgentID) {
		return fmt.Errorf("authority: fleet state mutation target is not actively enrolled")
	}
	snapshot, found, err := manager.StateSnapshot(ctx, mutation.TenantID, mutation.AgentID)
	if err != nil || !found || snapshot.Revision != mutation.ExpectedRevision {
		return fmt.Errorf("authority: fleet state mutation expected revision is not current")
	}
	requiresEntitlement, err := manager.requiresFleetEntitlement(ctx, mutation.TenantID, []string{mutation.AgentID})
	if err != nil {
		return err
	}
	if requiresEntitlement && (manager.entitlements == nil || !manager.entitlements.Allows(ctx, mutation.TenantID, CommercialFleet)) {
		return ErrEntitlementRequired
	}
	if err := persistence.SaveFleetStateMutation(ctx, mutation); err != nil {
		return persistenceError("save fleet state mutation", err)
	}
	return nil
}

func (manager *FleetManager) StateMutations(ctx context.Context, tenantID, agentID string) ([]FleetStateMutation, error) {
	persistence, ok := manager.persistence.(FleetStatePersistence)
	if !ok {
		return nil, fmt.Errorf("authority: fleet state persistence is unavailable")
	}
	mutations, err := persistence.ListFleetStateMutations(ctx, tenantID, agentID)
	if err != nil {
		return nil, persistenceError("list fleet state mutations", err)
	}
	results, err := persistence.ListFleetStateMutationResults(ctx, tenantID, agentID)
	if err != nil {
		return nil, persistenceError("list fleet state mutation results", err)
	}
	completed := make(map[string]struct{}, len(results))
	for _, result := range results {
		completed[result.MutationID] = struct{}{}
	}
	current, found, _ := manager.StateSnapshot(ctx, tenantID, agentID)
	values := mutations[:0]
	for _, mutation := range mutations {
		if _, done := completed[mutation.ID]; done || mutation.ExpiresAt < manager.now().Unix() || found && mutation.ExpectedRevision != current.Revision {
			continue
		}
		publicKey, keyErr := manager.trust.DecisionKey(ctx, tenantID, mutation.KeyID)
		if keyErr == nil && mutation.Verify(publicKey, manager.now()) == nil {
			values = append(values, mutation)
		}
	}
	sort.Slice(values, func(i, j int) bool { return values[i].IssuedAt < values[j].IssuedAt })
	return values, nil
}

func (manager *FleetManager) ReportStateMutationResult(ctx context.Context, result FleetStateMutationResult) error {
	persistence, ok := manager.persistence.(FleetStatePersistence)
	if !ok {
		return fmt.Errorf("authority: fleet state persistence is unavailable")
	}
	...
	mutations, err := persistence.ListFleetStateMutations(ctx, result.TenantID, result.AgentID)
	if err != nil {
		return persistenceError("load fleet state mutation", err)
	}
	found := false
	for _, mutation := range mutations {
		if mutation.ID == result.MutationID && mutation.ExpectedRevision == result.FromRevision {
			found = true
			break
		}
	}
	if !found {
		return fmt.Errorf("authority: fleet state mutation result has no matching mutation")
	}
	if err := persistence.SaveFleetStateMutationResult(ctx, result); err != nil {
		return persistenceError("save fleet state mutation result", err)
	}
	return nil
}
